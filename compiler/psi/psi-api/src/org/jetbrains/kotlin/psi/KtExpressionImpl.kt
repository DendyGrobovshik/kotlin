...
abstract class KtExpressionImpl(node: ASTNode) : KtElementImpl(node), KtExpression {
    ...
    companion object {
        @Deprecated(
            "Use KtPsiMutationService.getInstance().replaceExpression(expression, newElement, reformat, rawReplaceHandler) instead",
            ReplaceWith("KtPsiMutationService.getInstance().replaceExpression(expression, newElement, reformat, rawReplaceHandler)"),
        )
        @OptIn(KtNonPublicApi::class)
