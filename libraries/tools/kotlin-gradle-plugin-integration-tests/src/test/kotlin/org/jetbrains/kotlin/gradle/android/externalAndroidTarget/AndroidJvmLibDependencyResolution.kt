...
class AndroidJvmLibDependencyResolutionIT : KGPBaseTest() {
    ...
    fun `test - project dependency resolves matching androidLibrary and jvm variants`(
        ...
        val producer = androidLibraryAndJvmProject(
            gradleVersion = gradleVersion,
        ...
        androidLibraryAndJvmProject(
            ...
            val resolvedVariants = buildScriptReturn {
                project.ignoreAccessViolations {
                    mapOf(
                        "androidCompileClasspath" to ProjectDependencyVariantSelector.selectedVariantDisplayName(
                            project = project,
                            configurationName = "androidCompileClasspath",
                        ),
                        "jvmCompileClasspath" to ProjectDependencyVariantSelector.selectedVariantDisplayName(
                            project = project,
                            configurationName = "jvmCompileClasspath",
                        ),
                        "commonMainMetadata" to ProjectDependencyVariantSelector.selectedVariantDisplayName(
                            project = project,
                            configurationName = "commonMainResolvableDependenciesMetadata",
                        ),
                        "androidRuntimeClasspath" to ProjectDependencyVariantSelector.selectedVariantDisplayName(
                            project = project,
                            configurationName = "androidRuntimeClasspath",
                        ),
                        "jvmRuntimeClasspath" to ProjectDependencyVariantSelector.selectedVariantDisplayName(
                            project = project,
    ...
    fun `test - published androidLibrary and jvm library publishes expected variants`(
        ...
        val producer = publishAndroidLibraryAndJvmProducer(
            gradleVersion = gradleVersion,
            androidVersion = androidVersion,
            jdkVersion = jdkVersion,
        ...
        val rootMetadataFile = producer.rootComponent.gradleMetadata
        val rootComponent = producer.rootComponent
        val jvmComponent = producer.jvmMultiplatformComponent
        val androidMultiplatformComponent = PublishedProject.Component(
            producer.repository.resolve(producer.group).resolve("${producer.name}-android").resolve(producer.version),
            "${producer.name}-android-${producer.version}",
        )

        listOf(
            rootMetadataFile to "published root metadata",
            rootComponent.pom to "published root pom",
            jvmComponent.gradleMetadata to "published jvm metadata",
            jvmComponent.pom to "published jvm pom",
            jvmComponent.jar to "published jvm jar",
            androidMultiplatformComponent.gradleMetadata to "published android metadata",
            androidMultiplatformComponent.pom to "published android pom",
        ).forEach { (file, description) ->
            assertFileExists(file, "Expected $description at $file")
        }

        val rootMetadataPath = rootMetadataFile.toPath()
        val expectedVariants = mapOf(
            "androidApiElements-published" to arrayOf(
            ...
            "androidRuntimeElements-published" to arrayOf(
                "org.gradle.category" to "library",
                "org.gradle.jvm.environment" to "android",
                "org.gradle.libraryelements" to "aar",
            ...
            "jvmApiElements-published" to arrayOf(
                "org.gradle.category" to "library",
            ...
            "jvmRuntimeElements-published" to arrayOf(
                "org.gradle.category" to "library",
                "org.gradle.jvm.environment" to "standard-jvm",
                "org.gradle.libraryelements" to "jar",
                "org.gradle.usage" to "java-runtime",
        ...
        expectedVariants.forEach { (variantName, expectedAttributes) ->
            assertGradleVariant(rootMetadataPath, variantName) {
                assertAttributesEquals(*expectedAttributes)
            }
        }
    }
    ...
    fun `test - published androidLibrary and jvm library exposes and resolves matching variants for KMP consumer targets`(
        ...
        androidLibraryAndJvmProject(
            ...
            sourceSets.getByName("androidJvmMain").dependencies {
                implementation(producer.rootCoordinate)
            }
            sourceSets.getByName("androidJvmMain").compileSource(
                """
                ...
                project.ignoreAccessViolations {
                    ...
                    configurationNames.associateWith { configurationName ->
                        project.configurations
                            ...
                            .getValue(producerRootCoordinate)
                            .configuration
                    }
                }
            }.buildAndReturn(":help")
                ...
                project.ignoreAccessViolations {
                    ...
                    configurationNames.associateWith { configurationName ->
                        project.configurations
                            .getByName(configurationName)
                            .resolveProjectDependencyComponentsWithArtifacts()
                            .getValue(producer.rootCoordinate)
                            .configuration
                    }
                }
            }.buildAndReturn(":help")
    ...
    private object ProjectDependencyVariantSelector {
        fun selectedVariantDisplayName(
            project: Project,
    ...
    private fun publishAndroidLibraryAndJvmProducer(
        gradleVersion: GradleVersion,
        androidVersion: String,
        jdkVersion: JdkVersions.ProvidedJdk,
        namespace: String,
    ): PublishedProject = androidLibraryAndJvmProject(
        gradleVersion = gradleVersion,
        androidVersion = androidVersion,
        jdkVersion = jdkVersion,
        namespace = namespace,
