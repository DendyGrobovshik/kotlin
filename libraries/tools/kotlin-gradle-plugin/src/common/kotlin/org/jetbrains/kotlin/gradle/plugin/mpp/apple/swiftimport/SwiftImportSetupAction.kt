    ...
    return registerTask<GenerateSyntheticLinkageImportProject>(
        ...
        it.syntheticImportProjectRoot.set(
            projectPathProvider.flatMap {
                project.layout.dir(
                    project.provider { syntheticImportProjectRoot(it).toFile() }
                )
            }
        )
        it.syntheticProductType.set(syntheticImportProjectProductType)
...
private fun syntheticImportProjectRoot(xcodeprojPath: String): Path {
    val projectPath = Paths.get(xcodeprojPath)
    return projectPath.parent.resolve(SYNTHETIC_IMPORT_TARGET_MAGIC_NAME)
}
