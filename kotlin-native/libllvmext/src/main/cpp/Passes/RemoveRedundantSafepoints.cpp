...
bool RemoveRedundantSafepointsPass::run(Function &F) {
  ...
  for (BasicBlock &BB : F) {
    ...
    if (HasSPs) {
      ...
      // If we can't yet remove all encountered prologue safepoints, preserve
      // the first one.
      if (CanRemoveAllSafepoints != CanRemoveAllSafepoints::Yes) {
        FirstSP = SPs.pop_back_val();
      }
      // All other prologue safepoints in this basic block can be safely
      // removed.
    ...
    if (CanRemoveAllSafepoints == CanRemoveAllSafepoints::Unknown) {
      // If the first basic block contains a prologue safepoint, all other
      // prologue safepoints can be removed.
